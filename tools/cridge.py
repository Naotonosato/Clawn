...
import subprocess
getpath =subprocess.Popen("llvm-config --libdir",shell=True,stdout=subprocess.PIPE)
libpath = getpath.stdout.read().decode().replace("\n","")
Config.set_library_path(libpath)
REAL_NUMBER_TYPES = set({
    TypeKind.DOUBLE,TypeKind.FLOAT,TypeKind.FLOAT128,TypeKind.LONGDOUBLE
})
INTEGER_TYPES = set({
...
def to_clawn_type(type_info):
    ...
    if type_info.kind == TypeKind.POINTER:
        ...
        if to_clawn_type(type_info) == "__C_VOID__":
            return "__PTR__ " * count + "__C_INT__SIZE_" + ptr_size
        return "__PTR__ " * count + to_clawn_type(type_info)
...
def generate_Ctype(structures):
    ...
    for name,struct in structures.items():
        ...
        if struct.get_size() > 0 and len(field) == 0: #unknown fields but has size
        ...
        for element in field:
            element_type_name = to_clawn_type(element["type"])
            ...
            if element_name == "" or ("(" in element_type_name): #functions are not supported yet
                element_name = f"__NOT_SUPPORTED__{get_unique()}"
            C_type_wrapper += member_template.format(
                element_name,
                element_type_name
                )
...
"""
    ...
    for func_name,info in functions.items():
        ...
        args_text = ",".join([to_clawn_type(t) for t in types])
        return_text = to_clawn_type(return_type)
...
if __name__ == "__main__":
    ...
    output_filename = os.path.splitext(os.path.basename(source_filename))[0] + ".cridge"
